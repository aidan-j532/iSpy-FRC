...
from iSpy.plugins.bases import FrameProcessorBase, TrackerBase, UtilityBase
...
class PluginStatusModuleTests(unittest.TestCase):
    ...
    def test_available_lists_all_addons_with_schemas(self):
        ...
        self.assertTrue(
            any(
                p["builtin"] and p["type"] == "vision_pipeline"
    ...
    def test_toggle_unknown_addon_404(self):
        ...
        with (
            ...
            mock.patch(
                "iSpy.web.Backend.PluginStatus.request",
                _FakeRequest.get_json(
                    {"name": "nope", "type": "tracker", "enable": True}
                ),
            ),
        ):
            resp = mod._toggle()
    ...
    def test_toggle_vision_pipeline_rejected(self):
        ...
        with (
            ...
            mock.patch(
                "iSpy.web.Backend.PluginStatus.request",
                _FakeRequest.get_json(
                    {"name": "april_tag", "type": "vision_pipeline", "enable": True}
                ),
            ),
        ):
            resp = mod._toggle()
    ...
    def test_save_settings_unknown_addon_404(self):
        ...
        with (
            ...
            mock.patch(
                "iSpy.web.Backend.PluginStatus.request",
                _FakeRequest.get_json(
                    {"name": "nope", "type": "tracker", "settings": {}}
                ),
            ),
        ):
            resp = mod._save_settings()
    ...
    def test_delete_disables_then_removes_file(self):
        ...
        try:
            ...
            self.assertNotIn("delete_me_test", cfg.config["plugins"]["trackers"])
        finally:
    ...
    def test_resolve_safe_path_never_reaches_builtin_dir(self):
        mod, cfg = self._module()
        for filename in (
            "BuiltIn/ObjectTracker",
            "BuiltIn/ObjectTracker.py",
...
class AllBuiltinsEnabledBootTests(unittest.TestCase):
    """Enabling every built-in utility/tracker/
