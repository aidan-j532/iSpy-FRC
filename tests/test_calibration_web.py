...
class _FakeCamera:
    def __init__(self, name):
        self.name = name
        self.config = type(
            "Cfg", (), {"get": lambda self, k, d=None: {"name": name}.get(k, d)}
        )()
    ...
    def in_calibration_mode(self):
        return bool(
            ...
            and time.monotonic() - self.calibration_last_seen < 10.0
        )
...
class CalibrationWebTests(unittest.TestCase):
    ...
    def test_charuco_capture_finish_flow(self):
        ...
        for i in range(4):
            r = client.post(
                "/api/cameras/calibration/cam_0/charuco/capture",
                json={
        ...
        self.assertEqual(
            cfg.config["camera_configs"]["cam_0"]["calibration"]["rms"],
            j["result"]["rms"],
        )
    ...
    def test_charuco_overlay_feed_draws_detected_board(self):
        ...
        served = self._first_jpeg(
            mod._generate_calibration("cam_0", overlay="charuco", pattern=(7, 5))
        )
        b, g, r = (
            served[:, :, 0].astype(int),
            served[:, :, 1].astype(int),
        ...
        green = (g > r + 20) & (g > b + 20)
        self.assertTrue(
            bool(green.any()), "expected green detection overlay on the served frame"
        )
    ...
    def test_charuco_overlay_feed_auto_detects_non_default_layout(self):
        ...
        green = (g > r + 20) & (g > b + 20)
        self.assertTrue(
            bool(green.any()), "expected auto-detected overlay for a 5x7 board"
        )
    ...
    def test_charuco_captured_overlay_drawn_in_its_color(self):
        ...
        self.assertTrue(
            bool(painted.any()),
            f"expected captured overlay in color {color} on the feed",
        )
    ...
    def test_pnp_save_and_clear_persists(self):
        ...
        self.assertEqual(
            j["pnp"]["camera_matrix"][0][0],
            300.0,
        )
        stored = cfg.get("camera_configs", {}).get("cam_0", {})
        self.assertEqual(
            stored["pipeline"]["settings"]["vision_model"]["pnp"]["object_points"],
            points,
        )
