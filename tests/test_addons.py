        ...
        for subdir, base in (
            ...
            for name, cls in load_plugins(PLUGIN_ROOT / subdir, base).items():
                for key, value in cls.default_settings().items():
                    self.assertEqual(cls.config_schema()[key]["default"], value, name)
...
class ObjectTrackerTests(unittest.TestCase):
    ...
    def test_custom_settings_apply(self):
        tracker = self._make({"distance_threshold": 0.9, "stale_threshold": 2.5})
    ...
    def test_nearby_detections_merge(self):
        tracker = self._make()
        fuels = tracker.update([Object(0.0, 0.0), Object(0.1, 0.05)], 0, 0, 0)
...
class PathPlannerTests(unittest.TestCase):
    ...
    def test_run_returns_clusters(self):
        planner = self._make({"epsilon": 0.3, "min_samples": 3})
        planner.update([Object(0.0, 0.0), Object(0.1, 0.0), Object(0.2, 0.0)], 0, 0, 0)
...
class RollBackTests(unittest.TestCase):
    ...
    def test_enabled_by_presence_records_frames_to_disk(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertGreater(
                files[0].stat().st_size, 24, "recording file must contain frames"
            )
...
class HealthModuleTests(unittest.TestCase):
    ...
    def test_camera_status_marks_stale(self):
        mod = self._make(cameras=[self.FakeCamera("a", 0.1), self.FakeCamera("b", 5.0)])
    ...
    def test_only_utilities_and_pipelines_contribute(self):
        vision = mock.Mock()
        vision.utilities = {
            "util": mock.Mock(get_health=lambda: {"state": "up", "metrics": []})
        }
        vision.trackers = {
            "tracker": mock.Mock(get_health=lambda: {"state": "x", "metrics": []})
        }
        vision.frame_processors = {
            "fp": mock.Mock(get_health=lambda: {"state": "x", "metrics": []})
        }
        mod = self._make(vision=vision)
        payload, _healthy = mod._build_payload()
...
class ExampleAddonTests(unittest.TestCase):
    ...
    def test_example_utility_declares_and_publishes_output(self):
        ...
        util.update(frame_data)
        self.assertEqual(frame_data["addon_data"]["example_output"], 2)
