...
class PipelineConfigTests(unittest.TestCase):
            ...
            data = {
                "camera_configs": {
                    "cam_detect": {
                        ...
                        "vision_model": {
                            "file_path": "YoloModels/pytorch/a.pt",
                            "min_conf": 0.3,
                        },
                        "target_format": "onnx",
                        "stale_flat_junk": 1,
    ...
    def test_object_detection_settings_not_globally_required(self):
        ...
        self.assertEqual(cam_cfg.get_pipeline_setting("tag_size_inches"), 6.5)
    ...
    def test_legacy_top_level_vision_model_migrates_to_cameras(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            data = {
                "vision_model": {
                    "file_path": "YoloModels/pytorch/legacy.pt",
...
class PipelineLifecycleTests(unittest.TestCase):
    ...
    def test_get_status_collapses_build_and_generic_run(self):
        pipeline = ConcretePipeline.__new__(ConcretePipeline)
        pipeline._set_status("optimizing (onnx build)")
        pipeline._set_status("initializing")
        self.assertEqual(pipeline.get_status(), "optimizing (onnx build)")
        self.assertEqual(pipeline.get_state(), "optimizing")

        pipeline._set_status("downloading (model weights)")
        self.assertEqual(pipeline.get_status(), "downloading (model weights)")
        self.assertEqual(pipeline.get_state(), "downloading")

        pipeline._set_status("Needs Calibration for Better Accuracy")
        self.assertEqual(
            pipeline.get_status(),
            "downloading (model weights)\nNeeds Calibration for Better Accuracy",
        )

        pipeline._set_status("ready")
        self.assertEqual(pipeline.get_status(), "ready")
        self.assertEqual(pipeline.get_state(), "ready")
    ...
    def test_only_object_detection_offers_optimization_options(self):
        ...
        self.assertFalse(
            ConcretePipeline.__new__(ConcretePipeline).get_optimization_options()
        )
        self.assertEqual(
            ConcretePipeline.__new__(ConcretePipeline).optimize(), "not supported"
        )
    ...
    def test_stale_artifact_resync_hooks_exist_on_all_model_backed_pipelines(self):
        ...
        for cls in (ObjectDetectionPipeline, YoloWorldPipeline, DepthAnythingPipeline):
            ...
            for hook in hooks:
                owner = next((k for k in cls.__mro__ if hook in vars(k)), None)
                self.assertIsNotNone(
                    owner, f"{cls.__name__} missing resync hook {hook}"
                )
                self.assertIsNot(
                    owner,
...
class BootTests(unittest.TestCase):
    ...
    def test_broken_camera_does_not_block_healthy_cameras(self):
        ...
        with mock.patch("iSpy.vision.pipelines.get_pipeline_classes", fake_classes):
            ...
            cam_cfg = iSpyCameraConfig(
                {
                    ...
                    "pipeline": {
                        "name": "object_detection",
                        "settings": {
                            "vision_model": {"file_path": missing, "source_pt": missing}
                        },
                    },
                }
            )
...
class QuantizeDatasetTests(unittest.TestCase):
    def test_default_dataset_dir_is_reusable_default(self):
        ...
        self.assertEqual(
            default_quantization_dataset_dir().parent.name, "QuantizeDataset"
        )

    def test_datasets_module_creates_default_folder(self):
        with tempfile.TemporaryDirectory() as tmp:
            with mock.patch(
                "iSpy.web.modules.datasets.Path.cwd", return_value=Path(tmp)
            ):
                ...
                self.assertFalse(
                    (Path(tmp) / "Config" / "active_dataset.json").exists()
                )
            ...
            with mock.patch(
                ...
                with app.app_context():
                    with mock.patch("iSpy.web.modules.datasets.request", FakeRequest()):
                        resp = mod._create()
                    result, status = (
                        (resp[0], resp[1]) if isinstance(resp, tuple) else (resp, 200)
                    )
...
class CameraOpenBoundedTests(unittest.TestCase):
    ...
    def _mk_cam(self):
        ...
        return Camera(
            {"name": "open_test", "source": "missing_frame_test.png"}, (640, 480), False
        )
...
class MDNSHostnameTests(unittest.TestCase):
    ...
    def test_env_override_wins(self):
        from iSpy.boot.setup_service import MDNS_HOSTNAME_ENV, default_mdns_hostname

        with mock.patch.dict(
            "os.environ", {MDNS_HOSTNAME_ENV: "my-coproc"}, clear=False
        ):
...
class ServiceUserTests(unittest.TestCase):
    def test_sudo_invocation_uses_sudo_user(self):
        ...
        with mock.patch(
            "iSpy.boot.setup_service.os.geteuid", return_value=0, create=True
        ):
    ...
    def test_non_root_uses_invoking_user(self):
        ...
        with mock.patch(
            "iSpy.boot.setup_service.os.geteuid",
            side_effect=OSError("n/a"),
            create=True,
        ):
            with mock.patch(
                "iSpy.boot.setup_service.getpass.getuser", return_value="aidan"
            ):
    ...
    def test_undetected_platform_falls_back_with_warning_stamp(self):
        ...
        with mock.patch.dict("os.environ", {}, clear=False):
            with mock.patch.object(
                opt, "_detect_rknn_target_platform", return_value=None
            ):
...
class ConvertModelSubprocessTests(unittest.TestCase):
    def test_quantized_kill_retries_unquantized(self):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if payload["quantize"]:
                return mock.Mock(returncode=-9, stdout="", stderr="")
            result_path = Path(str(args_path) + ".result.json")
            result_path.write_text(
                json.dumps({"result": "Outputs/model_unquantized.rknn"})
            )
        ...
        self.assertTrue(calls[0]["quantize"])
    ...
    def test_nonquantized_failure_falls_back_without_retry(self):
        ...
        self.assertEqual(out, Path("models", "yolov8s.pt"))
    ...
    def test_data_yaml_written_with_metadata_classes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(
                meta,
                "read_metadata",
                return_value={"nc": 3, "names": {0: "a", 1: "b", 2: "c"}},
            ):
...
class PipelineCalibrationGateTests(unittest.TestCase):
    ...
    def _april_cam(self, calibration):
        ...
        cam.get_frame = lambda: __import__("numpy").zeros(
            (40, 40, 3), dtype=__import__("numpy").uint8
        )
        cam.logger = None
        return cam
    ...
    def test_section_satisfied_rules(self):
        self.assertTrue(
            VisionPipeline._section_satisfied(
                "charuco",
                {
