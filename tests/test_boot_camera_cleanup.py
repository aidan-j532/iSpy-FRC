...
class BootCameraCleanupTests(unittest.TestCase):
    ...
    def test_bare_index_without_device_id_is_kept(self):
        ...
        with (
            ...
            cleanup_missing_cameras(cfg)
            self.assertEqual(
                self._saved_keys(cfg),
                {"default_cam", "maybe_gone", "known"},
            )
    ...
    def test_no_devices_detected_skips_cleanup(self):
        with tempfile.TemporaryDirectory() as tmp, _isolated_save_dir(), _probed([]):
            cfg = self._make_config(
                tmp,
                {
            ...
            cfg = self._make_config(
                tmp,
                {
                    ...
                    "second": _cam(
                        "second",
                        source=1,
                        device_id="##?#USB#VID_AAAA&PID_BBBB&MI_00#1&x",
                    ),
                },
            )
            cleanup_missing_cameras(cfg)
