...
if _HAS_TORCH:
    from iSpy.vision.yolo_pt import load_yolo_pt, register_shim
...
class UltralyticsRemovalTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        if not _DEFAULT_DETECT_PT.exists():
            raise unittest.SkipTest(
                f"Required test model not found: {_DEFAULT_DETECT_PT}"
            )
    ...
    def test_load_yolo_pt_without_ultralytics_import(self):
        ...
        with mock.patch.dict(
            sys.modules, {"ultralytics": None, "ultralytics.nn": None}
        ):
...
class PoseModelRegressionTests(unittest.TestCase):
    """_default_pose.pt loads, infers, and returns keypoints.
    ...
    def setUpClass(cls):
        if not _DEFAULT_POSE_PT.exists():
            raise unittest.SkipTest(
                f"Required test model not found: {_DEFAULT_POSE_PT}"
            )
    ...
    def test_pose_pt_loads_and_infers_with_keypoints(self):
        ...
        self.assertIsNotNone(
            r.keypoints,
            "synthetic frame produced no candidate - pose decode never exercised",
        )
        kd = r.keypoints.data
...
class CameraRestructureTests(unittest.TestCase):
    ...
    def test_camera_facade_reexports(self):
        from iSpy.vision.Camera import CameraBase as FacadeBase
        from iSpy.vision.Camera import create_camera as FacadeCreate
        from iSpy.vision.Cameras import CameraBase as RealBase
        from iSpy.vision.Cameras import create_camera as RealCreate
