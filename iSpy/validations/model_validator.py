...
def validate_config_model_paths(
    ...
    if matches:
        ...
        logger.warning(
            f"Config model path mismatch, specified: {config_model_path} but found: {rel_path}. Using found path."
        )
        return rel_path
    else:
...
def cmd_validate_models() -> int:
    ...
    if result.is_valid():
        ...
        if result.orphan_models:
            logger.warning(
                "Found orphan/standalone models - cannot infer YOLO parameters"
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ["validate-models", "check-org"]:
        ...
        print("Usage: python -m iSpy.validations.model_validator validate-models")
        print("       python -m iSpy.validations.model_validator check-org")
