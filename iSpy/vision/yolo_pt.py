# DISCLAMER: This file, and similar related files, are generated by AI for a replacement to Ultralytics.
...
def _scale_boxes(
    ...
    if ratio_pad is None:
        ...
        gain = (
            ratio_pad[0][0] if isinstance(ratio_pad[0], (tuple, list)) else ratio_pad[0]
        )
        pad = ratio_pad[1]
    if padding:
        boxes[..., 0] -= pad[0]
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # per image
        ...
        for c in cls.unique():
            ...
            output_boxes.append(
                torch.cat([bx[keep], bscores[keep][:, None], bcls[keep][:, None]], 1)
            )
        if not output_boxes:
            continue
...
def _pose_nms(prediction, kpts, conf_thres, nc, num_keypoints, keypoint_dims):
    ...
    kp_out = [
        torch.zeros((0, num_keypoints, keypoint_dims), device=prediction.device)
        for _ in range(bs)
    ]
    ...
    for xi in range(bs):
        ...
        det = torch.cat(
            [x[keep_idx, :4], scores[keep_idx, None], cls[keep_idx, None]], 1
        )
        kp = kpts[xi, :, keep_idx]  # [nk, N_keep]
        kp = kp.view(num_keypoints, keypoint_dims, -1).permute(
            2, 0, 1
        )  # [N_keep, K, dims]
        dets[xi] = det
        kp_out[xi] = kp
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, reg_max=16, end2end=False, ch=()):
        ...
        c2, c3 = (
            max((16, ch[0] // 4, self.reg_max * 4)) if ch else 64,
            (max(ch[0], min(self.nc, 100)) if ch else 80),
        )
...
class YoloPT:
    def __init__(
        self, model: DetectionModel, names: dict, task: str, nc: int, imgsz: int = 640
    ):
        self.model = model
        self.names = names
        self.task = task
    ...
    def _letterbox(self, img, new_shape=640, stride=32, pad=114):
        ...
        h, w = img.shape[:2]
        new_shape = (
            (new_shape, new_shape) if isinstance(new_shape, int) else tuple(new_shape)
        )
        ...
        for xi, det in enumerate(dets):
            ...
            if det.numel():
                ...
                if kpts_all[xi] is not None:
                    kpts_i = _scale_keypoints(
                        kpts_all[xi], frame_shape, self._ratio, self._pad
                    )
            orig = (int(frame_shape[0]), int(frame_shape[1]))
            results.append(
                _Result(
                    _Boxes(det),
                    _Keypoints(kpts_i) if kpts_i is not None else None,
                    orig,
                )
            )
        return results
...
def cv2_copyMakeBorder(img, top, bottom, left, right, pad):
    ...
    return cv2.copyMakeBorder(
        img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(pad, pad, pad)
    )
